...
class TreePrinter:
    ...
    def print_tree(self, indent=0):
        tree = ""
        for instruction in reversed(self.instructions):
            tree += delimiter_char * indent + instruction.print_tree(indent) if instruction.print_tree(indent) is not None else ""
    ...
    def print_tree(self, indent=0):
        return delimiter_char * indent + "PRINT " + self.print_arg.print_tree(indent + 1) + "\n"

    @addToClass(classes.PrintIdList)
    def print_tree(self, indent=0):
        if self.print_ids is None:
            return str(self.single_id) #.print_tree(indent)
        else:
            #return self.single_id.print_tree(indent) + self.print_ids.print_tree(indent)
            return str(self.single_id) + " " + self.print_ids.print_tree(indent)
    ...
    @addToClass(classes.ExpressionAssignment)
    def print_tree(self, indent=0):
        tree = "=\n"
